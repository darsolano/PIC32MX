...
int main(void)
{
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // STEP 2. setup the PMP
    mPMPOpen(CONTROL, MODE, PORT, INTERRUPT);

    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // STEP 3. demonstrate a simple write operation (16-bit) value to
    // external memory device address 0x4000
    #define EXT_ADDRS               0x4000
    unsigned short value;

    PMPSetAddress(EXT_ADDRS);
    PMPMasterWrite(0x1234);

    // The first read flushes then latches data from external device into PMP
    PMPMasterRead();

    // now read the data from previous read operation value should = 0x1234
    value = PMPMasterRead();

    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // STEP 4. demonstrate a block write of 1024 half Words (16-bit) values to
    // external memory device address starting at 0x5000.
    // Note: readBuf and writeBuf are assumed to be initialized in this example.

    #define EXT_BLOCK_ADDRS         0x5000
    unsigned short writeBuf[1024];
    unsigned short readBuf[1024];

    PMPMasterWriteWordBlock(EXT_BLOCK_ADDRS,1024, writeBuf);
    PMPMasterReadWordBlock(EXT_BLOCK_ADDRS, 1024, readBuf);
    // values in readBuf should == value in writeBuf

    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // STEP 5. close peripheral
    mPMPClose();
